...
| [ml/paper/notes/family1gf.pdf](ml/paper/notes/family1gf.pdf) | Dataset design note for family 1.gf, the proposed GLOBAL fine-granularity observation family (≤ 10 km, ≤ 5 days, raw observations first): the ledger of every candidate checked against its producer on 2026-09-16 (daily 4 km ocean colour, 5 km and 2 km level-3 SST, 6 km sea ice, 2 km SWOT swaths, 4 km half-hourly cloud-top temperature, ICOADS ship reports, the World Ocean Database, GLODAP, BGC-Argo, OceanSITES, column CO₂), what fine scale buys each of the four goals, the sharded tiled compressed layout for grids finer than 0.25°, the row and byte arithmetic per store, the two-track licence rule (public Hub stores by default; sources that forbid redistribution become PRIVATE stores in a private repository, read by one job at a time, used here and never republished), a three-phase build ladder, the falsifier per store, the named exceptions (12.5 km wind, IMERG, GOSAT) and the decisions left open |
| [ml/paper/notes/family1tf.pdf](ml/paper/notes/family1tf.pdf) | Dataset design note for family 1.0.tf, the proposed TERRESTRIAL + COASTAL fine-granularity family, in the family-7 shape: purpose (land change as a prediction target — deforestation, biomass and canopy structure, floods, fire, crops, cities — plus the land as forcing and the coast as an AMOC / El Niño read-out); the rule, the ±100 km coastal band and the two-track licence rule (public Hub stores where redistribution is allowed; ISMN, full GRDC, ECA&D, the research-only tide-gauge sources and NICFI built as PRIVATE stores, used here but never republished); the stores by tier — station networks (GHCNh, GHCN-Daily, IGRA), tide gauges, buoys, HF radar, gliders, flux towers, fire detections, river gauges; GEDI and ICESat-2 LiDAR footprints with the vertical profile as channels; daily 0.05° land fields; the scene catalogue across forty years of Landsat, Sentinel-1/2, HLS, NISAR, PALSAR, OLCI, VIIRS and open sub-metre orthophotos; the read-out targets (disturbance alerts, loss year, burned area, floods, biomass, land cover); design decisions (native vertical structure, state and event both stored so rates are measurable, one catalogue across sensors never blended), validation, limitations, the ladder and open decisions |
| [ml/paper/notes/family09tf.pdf](ml/paper/notes/family09tf.pdf) | Dataset design note for family 0.9.tf: family 1.0.tf with ONE substitution — Google's published AlphaEarth Satellite Embedding V1 (64-d unit vectors per 10 m pixel per year, 2017–2025, CC BY 4.0, 577 TB on two public buckets) in place of the raw-imagery catalogue — and every other 1.0.tf store (point networks, LiDAR, daily fields, targets, statics) inherited by reference through an `inherits` block, same files and checksums, nothing rebuilt: what it inherits, what the embedding is (inputs, training targets, int8 companding, the annual valid period), the strict availability rule that keeps a year's summary out of its own anchors, three copied stores (a 1 km pooled field with a coherence channel, raw 10 m samples at fixed sub-cell positions, 100 m over regions of interest) plus the 10 m original referenced by byte range, the one-pass build next to the data, validation, limitations, and the three falsifiers that decide it against family 1.0.tf's own codec |
| [ml/paper/notes/family1gf_summary.pdf](ml/paper/notes/family1gf_summary.pdf) | Family 1.gf in two pages, in plain English: what the global fine-granularity family is, what it is for and what it is not; one table of all 16 proposed stores with a one-line description, producer, spatial and temporal granularity, record span, stored size (the one-month probe measurements where they exist), public/private track and build phase; how the stores are laid out; the three-phase ladder; then a full alphabetical glossary of every term, acronym, producer, instrument and code name in the summary and the design note |
| [ml/paper/notes/family1tf_summary.pdf](ml/paper/notes/family1tf_summary.pdf) | Family 1.0.tf in two pages, in plain English: the land-and-coast family's purpose and limits; one table of all 41 proposed stores and additions (point networks, LiDAR, daily 0.05° fields, the image catalogue, read-out targets, statics, phase-C additions) with description, producer, space and time granularity, record, stored size (probe measurements where they exist), track and phase; the storage layout; the ladder; then the full glossary |
| [ml/paper/notes/family09tf_summary.pdf](ml/paper/notes/family09tf_summary.pdf) | Family 0.9.tf in two pages, in plain English: family 1.0.tf with its raw imagery replaced by the AlphaEarth embedding; one table of the four AlphaEarth stores and the 29 stores inherited from 1.0.tf (each marked), with description, producer, granularity, record, size, track and phase; the registry's `inherits` block and the pooled-embedding layout; the ladder; then the full glossary |
