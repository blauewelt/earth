...
| `xco2` | 1.gf | 18 hosted year lanes #433–#450 (2009 … 2026-01-01..09-30, `--push-parts`, 2–16 min each) fetched and parked the record's parts; 2011 (#435) and 2022 (#446) died on a `ConnectTimeout` to URS (NASA's Earthdata login server, which every protected download passes through) and were re-dispatched as #457 and #458; assembly **#477** (hosted, the whole record `stage=all --parts-from-hub --assemble streaming` 2009-01-01..2026-09-30, 25 min) | 511,716,245 | 16,886,659,047 (16.89 GB, 10 files) | 2009–2026 (bins 1993–3256, 1,262 of 1,264 live; 18 years with rows) | 2026-09-23 | Satellite column CO2 — XCO2, the dry-air CO2 fraction averaged over the whole atmospheric column, in ppm — from NASA's OCO-2 and OCO-3 (Orbiting Carbon Observatory) Lite files, with Japan's GOSAT (Greenhouse gases Observing SATellite) through NASA's ACOS retrieval as a third sensor code (exception E3); public; schema 2, C = 3 (`xco2`, `xco2_uncertainty`, `surface`). One row per GOOD sounding: OCO-2 359,557,652, OCO-3 148,174,059, GOSAT-ACOS 3,984,534, out of 889,141,905 soundings in 12,293 of 12,293 files — 377,416,375 failed their retrieval's quality flag and 9,271 had no XCO2. **The record's shape is the launch dates**: 88,047–194,287 rows a year for 2009–2013 (GOSAT alone), 7.5 M in 2014 (OCO-2 from its first science data), 25.6 M in 2015, a peak of 60.2 M in 2020 with OCO-3 flying (from 2019-08), and 28.3 M for 2026 up to the archive's edge in bin 3256 (2026-07-29 … 08-02). The two empty bins are 1998 (2009-05-09 …) and 2414 (2015-01-18 …). XCO2 runs 364.25–450.25 ppm, mean 413.63; one value outside 300–500 ppm is NaN and counted. **The 1,089,888 `surface` values out of bounds (0.21 %) are not bad data**: the adapter passes `land_water_indicator` through unchanged in the 6,221 files that carry it and expects 0 land / 1 water, but the Lite product's flag also takes 2 (inland water) and 3 (mixed land and water). Anything outside the channel's 0–1 bounds becomes NaN and is counted, so these are soundings over lakes and coastlines whose surface class is missing from `values` (the `qc` water bit is still set for them, because the adapter tests ≥ 0.5 before the bounds apply). The other 6,072 files resolved to `Sounding/land_fraction`, a percentage the adapter maps to 0/1, which cannot fall outside. store.json holds sha256 for the nine arrays. The adapter projected ≈ 400 M good OCO-2 soundings plus OCO-3 and ≈ 18 GB, and the store is 16.89 GB |
| `pace4k` | 1.gf (tier G, sharded) | hosted year lanes #352 (2024), #353 (2025) and #354 (2026 to September), 42–74 min each on 2026-09-20; ledger repair **#430** (`--stage repair` rebuilt 2024's overwritten year ledger from the shards present, 3 min); assembly **#478** (hosted, `stage=all --parts-from-hub` 2024-03-05..2026-09-30, 1 h 39 min) | 857 daily frames in 177 bins (325,553 tiles) | 21,374,437,705 (21.37 GB, 356 files) | 2024-03 → 2026-07 (bins 3080–3256) | 2026-09-23 | Ocean colour and the phytoplankton community from PACE (NASA's Plankton, Aerosol, Cloud, ocean Ecosystem satellite), daily 4 km mapped v3.2 on `oc4k`'s grid, C = 7: `log_chl` (log10 chlorophyll-a), `poc` (particulate organic carbon), `carbon_phyto` (phytoplankton carbon), `avw_400` (the apparent visible wavelength minus 400 nm) and three cell abundances inferred by the MOANA algorithm (`pro_moana`, `syn_moana`, `pico_moana` — a level-4 DERIVED product, regional to 70 S–70 N, 85 W–25 E); public. 857 of 945 frames wanted (292 in 2024, 360 in 2025, 205 in 2026). Missing 88: 18 absent upstream; 4 before the record (2024-03-01..04, and the record opens 03-05); 66 after it. 62 of those 66 are 2026-08-01 … 10-01, because the window ran to 2026-09-30 past the archive's live edge (its last day was 2026-07-31), so 12 bins (3257–3268) lie wholly outside the record (`bins_outside_record`). The other 4 are 2026-01-01..04 in bin 3214: the 2025 lane, run before the year-boundary fix 77dff61, wrote its copy of that bin over the 2026 lane's and marked those days `after_record`, and recovering them is a re-fetch of 2025. 1,678 files read through OB.DAAC's OPeNDAP with no login, 26.77 GB and 6,358 s of fetch; MOANA was not published on 17 days. Valid fraction 0.085 for the four global channels and 0.020 for the three MOANA ones. 375 `pico_moana` and 33 `syn_moana` values above the channel's 60,000 (thousand cells per mL) are NaN and counted. Values above the producer's own valid_max but inside the bounds are kept and counted (picoeukaryotes 4,414,993). The adapter's three-day measurement projected ≈ 23 GB for the record, and the store is 7 % under it |
| `irtb` 2015 | 1.gf (tier G, sharded) | four hosted quarter lanes #451–#454 (2015 q1–q4, first fetch, 13–18 min each); first assembly #460 (hosted, 23 min) came out **28 frames short** at 2,892: every lane's last bin lost its last seven three-hourly frames to `after_record` because the catalogue query ended on a date rather than on the bin's end (fixed in 06ebafe); re-fetch #467–#470 (the same four quarters with `--force` on the fixed code, 13–18 min); re-assembly **#479** (hosted, 26 min) | 2,920 three-hourly frames in 73 bins (786,115 tiles) | 17,689,709,368 (17.69 GB, 148 files) | 2015 (bins 2411–2483, 2015-01-03 → 2016-01-02) | 2026-09-23 | Cloud-top temperature: the merged infrared brightness temperature of NCEP/CPC (the US National Centers for Environmental Prediction's Climate Prediction Center), GPM_MERGIR v1, 4 km, built from the geostationary weather satellites; public; C = 1 (`tb`, stored as kelvin minus 160 in uint8). **This store is phase A, not the whole product**, as store.json's `notes` says: 30 S–30 N only (1,649 × 9,896, rows 824–2473 of the 60 S–60 N grid) and three-hourly (every sixth half-hour, 40 frames a bin). The full half-hourly form needs 240 frames a bin against the layout's 64, so it is a change to the layout, not a re-run. 2,920 of 2,920 frames, none missing, from 2,920 hourly files (each holding two half-hourly steps, checked on every file); 82.11 GB read, and the lanes' ledgers sum to 2,587 s of fetch. A bin belongs to the year its first day falls in, so the store runs 2015-01-03 00Z → 2016-01-02 21Z, and 2015-01-01..02 sit in 2014's bin 2410. Valid fraction 0.9659. 3,148 `tb` values outside 160–414 K are missing and counted, never clipped. `lanes_by_year` declares q1–q4 |
| `sst_acspo02` 2020 | 1.gf (tier G, sharded) | two hosted half-year lanes #455 (2020-01-01..06-30) and #456 (07-01..12-31), ≈ 3 h 08 m each at ≈ 58 s a frame; assembly **#492** (hosted, `stage=all --parts-from-hub --lanes d0101-0630,d0701-1231 --assemble streaming` 2020-01-01..2020-12-31, 1 h 51 min) | 365 daily frames in 73 bins (647,797 tiles) | 19,284,014,626 (19.28 GB, 148 files) | 2020 (bins 2776–2848, 2020-01-02 → 2020-12-31) | 2026-09-23 | Sea-surface temperature from NOAA's ACSPO (Advanced Clear-Sky Processor for Ocean) L3S-LEO DY product: the polar orbiters' day and night passes super-collated into one daily granule, 0.02°, 9,000 × 18,000, v2.81; public; C = 2 (`sst` in °C, `quality_level`). 365 of 365 frames. The bins are the ones whose first day is in 2020, so 2020-01-01 belongs to 2019's bin 2775. 365 granules read, 79.04 GB; the two lanes' ledgers sum to 21,300 s of fetch (58.4 s a frame). CMR (NASA's granule catalogue) declared 180 of the granules at 108 bytes, and each of them read 416–470 MB, which is why a declared size is no longer a refusal. Valid fraction 0.354. Every graded pixel of the year is grade 0 (37.54 × 10⁹) or grade 5 (20.94 × 10⁹) and nothing in between, as the 18-day probe found, so `quality_level` is a constant 5 wherever `sst` exists: a mask, not a grade. 11,313 `sst` values outside −2.5–40 °C are NaN and counted. The #172 probe projected ≈ 18.9 GB for a year, and the store is 2 % over it |
...
### 2026-09-23 · four 1.gf stores landed: `xco2`, `pace4k`, `irtb` 2015, `sst_acspo02` 2020

All four rows are in the table at the top. Every number there comes from the
published `store.json` and `manifest.json`. `xco2` is the first whole-record
1.gf point store; it was assembled on a hosted runner from 18 year lanes in
25 min. `pace4k` is whole from 2024-03-05 to the archive's live edge
(2026-07-31) except 4 days, which a re-fetch of 2025 would recover. `irtb` and
`sst_acspo02` are one reference year each, and each was fetched in lanes. The
`irtb` year had to be fetched twice: the first assembly (#460) came out 28
frames short because of the last-bin bug, and 06ebafe fixed it. **Two
counts need reading, not alarm.** First, `xco2`'s 1.09 M out-of-bounds
`surface` values are the Lite flag's inland-water (2) and mixed (3) codes
landing outside a 0/1 channel. Second, `sst_acspo02`'s `quality_level` holds
only grade 5, so it is a mask.

## Notes — E-082 wave 8, the full 1.gf fetch (2026-09-23)

A session-side queue drives the fetch: `lane_queue.json` holds the lanes and
`topup.mjs` tops the hosted pool back up. Neither is in this repository.
There are 190 lanes: 111 `irtb` quarters, 52 `sst_acspo02` half-years and 27
`swot` months. At 15:20Z, 16 hosted lanes were in flight. At that load, three
`irtb` lanes died on a URS `ConnectTimeout` while fetching their first file:
#509 (2001 q3), #514 (2002 q1) and #520 (2002 q4). 6c4dca2 lengthened
`earthdata_download`'s retry ladder to six attempts with doubling back-off,
and all three went back into the queue (#509 is #521). **Fetch cannot run on
a rented box.** It needs the Earthdata credentials, which never go on a
