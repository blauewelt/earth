...
class ICESat2Adapter(f10b.SourceAdapter):
    ...
    def __init__(self):
        ...
        # THE CAP IS THE PROBE'S, NEVER THE BUILD'S. `ICESAT2_MAX_GRANULES` bounds
        ...
        # 2026-09-20 the first build lanes ran with the probe's default cap,
        # read twelve granules of 4,884, and MARKED THE YEAR DONE with a
        # fraction of it in 38 seconds -- a short store that reported success
        # (ml/CLAUDE.md 0.2). So `fetch_year` never applies a cap, and a cap
        # set explicitly for a build is a refusal unless
        # ICESAT2_ALLOW_CAPPED_BUILD=1 names the intention.
        raw_cap = (os.environ.get("ICESAT2_MAX_GRANULES") or "").strip()
        try:
            self.max_granules = int(raw_cap) if raw_cap else 12
        except ValueError:
            sys.exit("ICESAT2_MAX_GRANULES must be an integer")
        self.cap_was_set = bool(raw_cap)
        self.allow_capped_build = (
            os.environ.get("ICESAT2_ALLOW_CAPPED_BUILD") or "") == "1"
    ...
    def fetch_year(self, ctx, year):
        ...
        if self.cap_was_set and self.max_granules and not self.allow_capped_build:
            sys.exit(f"{self.store}: ICESAT2_MAX_GRANULES={self.max_granules} is "
                     ...
                     f"ICESAT2_ALLOW_CAPPED_BUILD=1 if a partial year is what "
