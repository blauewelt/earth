...
- Dispatch: `node scripts/family1_dispatch.mjs family1-build.yml '<inputs json>'` — inputs `store`, `stage`, `start`, `end`, `runner`, `extra_args`, `probe_month`, `check_credentials`, `adapter_env`. **`check_credentials:"true"` runs ONLY the Earthdata check and no probe or build.** **A fetch lane parks nothing unless `extra_args` carries `--push-parts`** (fifteen lanes were dispatched without it on 2026-09-20 and had to be cancelled and re-dispatched). **A lane inside a year is a NAMED lane** (landed 2026-09-20 evening, `tests/test_family1_lanes.py`, ADAPTER_CONTRACT.md "Lanes"): `start=2022-06-01 end=2022-06-30` writes lane `m06` of 2022 under `partials/<family>/<store>/2022/m06/` with its own parts, index, ledger and `done.json`; quarters are `q1`–`q4`, any window `d0701-0930`, a tile subset `g-<hash>` (`CANOPY30_TILES`, `LOSSYEAR_TILES`); the assembly declares the set with `--lanes months|quarters|<list>` and refuses a missing or overlapping lane. A named tier-G lane owns the bins that START in its window, so a laned tier-G assembly starts on the year's first bin start (`--start 2020-01-02` for 2020) or carries `--allow-missing-years`. Before this layout, two lanes in one year overwrote each other's index (ICESat-2's monthly lanes of 2026-09-20 17:52Z, cancelled before any part reached the Hub).
...
1b. The lane-aware parts layout LANDED (§7). Dispatch `icesat2` 2022 as twelve monthly lanes (`ICESAT2_FETCH=whole`, `--push-parts`, no cap variable) and assemble on the box with `--parts-from-hub --lanes months --assemble streaming`; `lai500` 2020 as four quarter-lanes (`LAI500_GROUPS=terra`) and assemble hosted with `--lanes quarters --start 2020-01-02`; `canopy30` as tile-subset lanes (`CANOPY30_TILES=…`, ~20 tiles a lane) and assemble with the whole tile list and `--lanes g-…,g-…`; `gedi` after its probe.
