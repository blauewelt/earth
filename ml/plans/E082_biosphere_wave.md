...
| `canopy30` | **the 2020 canopy-height map at 30 m.** GLAD Global Forest Canopy Height 2020 (Potapov et al. 2021), Landsat fitted to GEDI; uint8 metres, codes for water/no-data (C = 1); annual sharded grid on the Hansen 10° tile grid cut into 2.5° groups exactly as `lossyear` | G · 30 m, annual (one bin) | 2020 only; 280 land tiles | ≈ 60–100 GB (dense metres) | CC BY 4.0 · public | **A** | none (GLAD storage, public) |
...
| `sif` | **photosynthesis seen from orbit.** Solar-induced chlorophyll fluorescence: TROPOMI (Sentinel-5P) ungridded daily soundings (Caltech, Köhler et al. 2018; and ESA TROPOSIF from the S5P-PAL service): SIF at 743 nm and its daily-corrected value, 1-σ, cloud fraction, solar and viewing zenith (C = 6); OCO-2 and OCO-3 SIF Lite (GES DISC) as second and third platforms with the same channels | P · 7 × 3.5 km (TROPOMI), 1.3 × 2.25 km (OCO); log2 f_p = −2.3 / −3.9; instantaneous | 2018-03 → (TROPOMI), 2014-09 → (OCO-2), 2019-08 → (OCO-3); ≈ 5 × 10⁸ clear soundings a year | 60–160 GB for TROPOMI depending on the cloud cut, which the probe measures | Caltech: cite; ESA: Copernicus open; NASA open · public | **A** (TROPOMI now; OCO after the GES DISC approval) | none for TROPOMI; EDL + GES DISC for OCO |
| `lai500` (E7) | **leaf area and light absorbed, every 8 days.** MOD15A2H v061 (Terra), MYD15A2H (Aqua), VNP15A2H (VIIRS) 8-day leaf-area index, FPAR (fraction of absorbed photosynthetically active radiation), their standard deviations and quality (C = 4); the 8-day composite is an exception E7 to the five-day rule, carried as a `frame_table` like `chirps05`'s pentads | G · 500 m sinusoidal tiles, sharded on the native tile grid (the layout already holds a projected grid: `seaice_asi` is polar stereographic) | 2000-02 → (Terra), 2002-07 → (Aqua), 2012 → (VIIRS); 46 composites a year, 460 land tiles | ≈ 60 GB per sensor-year after compression | NASA open · public | **A** builds Terra 2020–2024 and measures; the record phase B | EDL (LP DAAC in place) |
| `pheno500` | **the leaf calendar, per year.** MCD12Q2 v061 land-surface phenology: greenup, mid-greenup, peak, senescence, mid-greendown, dormancy dates (day of year, int16), EVI minimum, amplitude, area, quality, first cycle (C = 10) | G · 500 m sinusoidal, annual | 2001–2023 | ≈ 10 GB per year | NASA open · public | **A** | EDL |
| `gbif` | **three billion species observations.** GBIF occurrence snapshot (parquet on the AWS Open Data registry, monthly): records with a date to the day or month and coordinates; time from the event date (noon when no time; f_t = one day, or one month for month-precision records); footprint from the coordinate uncertainty; platform = the hashed taxon key; kingdom code, class code, basis-of-record code, individual count (C = 4); `platforms.json` maps the hash to the taxon key and name; schema 3 | P · point; per-row footprint | 1600s →; ≈ 2.5 × 10⁹ dated georeferenced records | ≈ 90 GB public; the CC BY-NC records (est. 20–30 %) on the private track | per record: CC0 and CC BY → public; CC BY-NC → private | **A** | none (anonymous S3) |
...
|---|---|---|---|---|---|---|---|
| `pace4k` | **hyperspectral ocean colour and phytoplankton community.** PACE OCI level-3 daily 4 km bins (OB.DAAC): chlorophyll, apparent visible wavelength, particulate organic carbon, phytoplankton carbon, and the MOANA community product (Prochlorococcus, Synechococcus, picoeukaryote concentrations) (C = 7) | G · 4 km daily, sharded | 2024-03 → | ≈ 50 GB per year | NASA open · public | **A** | EDL (OB.DAAC in place) |
...
3. **GBIF's non-commercial records**: the private track as designed, or drop
   them (the public CC0/CC BY records are ≈ 70–80 % of the snapshot).
4. **ESA CCI Biomass terms**: public if the licence allows redistribution
   with attribution (the agent reads the terms and reports), else private.
