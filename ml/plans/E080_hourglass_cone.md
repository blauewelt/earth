...
ablation after the three arms (slide 17). Slides are now 20 — 21 with the one-slide summary added on 16 Sep (slide 21, which reserves space for the results figure).

**Revision 3.1 (16 Sep), after Chris asked whether time is sunflower-sampled:** it is not — the past cone is a 2-D sunflower stacked at every pentad, by design (the codec owns the last 30 days at full cadence; stage 2 owns the long window, [E-071 §4.5](https://blauewelt.github.io/earth/docs.html?f=ml/plans/E071_cone_v2.md)). Two cheap additions: the sunflower's **phase rotates by the golden angle per lag**, so the stacked slices interleave in space-time instead of repeating the same bearings (slide 8); and the per-channel aperture gains a **time axis** — a learned memory τ_c per channel, initialised from `FAMILIES.tau_days`, so "which lags does this channel read" is learned rather than typed (slides 9–10). A log-spaced inner time ramp is listed as an ablation, not the design (slide 19). The deck:
...
- **u_i**: the canonical sunflower on the unit disc (Vogel spiral, golden angle — the E-026 pattern in [`ml/temporal.py::spiral_offsets`](https://github.com/blauewelt/earth/blob/main/ml/temporal.py)), fixed, 24 points — **phase-rotated by the golden angle per lag** (bearing_i,ℓ = i·137.5° + ℓ·137.5°), so the six slices interleave in space-time instead of repeating the same bearings. Time itself stays dense: one slice per pentad, because pentads are the finest cadence we have and the 30-day window is the codec's whole job
...
2. **Per channel**: two log-parameters — a **scale** s_c (two for the L-shaped channels: lags 0–1 and 2–6) and a **memory** τ_c. In space, S_c(ℓ) = S(ℓ) + s_c·I, i.e. Σ_c(ℓ) = e^{s_c} · Σ(ℓ) — the same ellipse inflated or shrunk, **never turned**. In time, lag ℓ is weighted by e^{−ℓΔt/τ_c} — how far back this channel is worth reading. *One scale and one memory per channel; the aperture is a space-time object.*
3. **Initialised from the reach-and-memory table** in [`ml/cone.py::FAMILIES`](https://github.com/blauewelt/earth/blob/main/ml/cone.py): wind stress 500 km with τ = 10 days (so lags 0–1 count and lag 6 is at e^{−3}); currents and sea-surface height 129.6 km·(1 + ℓ) with a long memory; sea-surface temperature and mixed-layer depth max(that, 500 km) at lags ≤ 1, then the ocean's growth; land 400 km flat, long memory. At step 0 every channel reads the reach and the lags the current geometry gives it

*Notes.* This slide answers a question Chris asked: don't we need a scale for each channel? Yes — and the split is the whole design. What is shared is the direction: the drift toward the source and the elongation of the footprint along the jet. Those are properties of the flow. Temperature, salinity, sea-surface height and the currents themselves are all carried by the same water, so giving each of them its own arrow would multiply the parameters and let the model learn different "upstreams" for quantities that arrive from the same place. What is per channel is the scale. The current geometry module — the file `ml/cone.py`, whose `FAMILIES` table assigns every channel a propagation speed, a memory and a correlation length — already says the channels differ: wind stress reads five hundred kilometres at the two lags inside its ten-day memory and nothing beyond, the ocean channels grow at a hundred and thirty kilometres per pentad, sea-surface temperature and mixed-layer depth are L-shaped — wide at lags zero and one where the atmosphere stirs them, then the ocean's slow growth — and land is a flat four hundred kilometres because soil does not advect. So each channel gets a log-scale offset that inflates or shrinks the shared ellipse without turning it — two of them for the L-shaped channels, one for the first two lags and one beyond — and, since revision 3.1, a memory: a time constant that says how far back the channel is worth reading, applied as an exponential weight over lag. That second number is the same table's other column made learnable: wind stress today is hard-coded to read lags zero and one and nothing beyond, because its memory is ten days; with a learned memory that cut-off is a starting value, not a rule. Both start at the values the geometry module already uses, so at step zero every channel reads exactly the reach and the lags it reads today. In the picture: the left panel is one dot set with three apertures of different size but the same centre and orientation; the right panel shows the same dots weighted for two channels — for the currents only the inner dots count, for sea-surface temperature at lag one more of them do. How the weighting happens, and why it costs no tokens, is the next slide.
...
- Size: 64,800 cells × 3 groups × 8 numbers ≈ 1.6 M nominal, about half of it live (ocean maps over ocean, land maps over land), plus a scale s_c and a memory τ_c per channel (~100 numbers, global — slide 9) — cheap, and no compute
...
- **Log-space, with soft floors**: Σ₀, Σ_v, s_c and τ_c are stored as logarithms (slide 8), so Adam's step is a fixed fraction at any size; the waist's eigenvalues are floored at one cell (28 km) and Σ_v's at the design speed, both by a softplus offset in log space rather than a clip
...
- **Init**: **d** = 0; S₀ = the log of a 3-cell circle with the measured aspect 0.71 ([`ml/measure_flow_anisotropy.py`](https://github.com/blauewelt/earth/blob/main/ml/measure_flow_anisotropy.py)); Σ_v from the 0.3 m/s of [`ml/cone.py::FAMILIES`](https://github.com/blauewelt/earth/blob/main/ml/cone.py); s_c and τ_c from the same table per channel — the fixed hourglass exactly, so step 0 of the learned arm *is* the control
...
1. **Waist + future cone in the geometry** (`cone.py`): `waist_dots`, `future_dots` = reflected `inner_dots`, the per-lag golden-angle phase in `inner_dots`; `coverage_report` extended with the mirror identity (future set = −past set) and re-asserted per lag under the phase — pure numpy, CPU test
...
3. **The aperture gate** (`cone_codec.py`): per-channel space-time weights w_{c,i,ℓ} on the per-location candidate tokens, ring ungated; test: with every s_c → +∞ and τ_c → +∞ (infinite aperture) the forward pass equals the ungated model bit-for-bit, and the finite-difference gradient of the loss w.r.t. S₀, Σ_v, **d**, s_c, τ_c matches autograd
...
**Two settled, seven still open — all reversible**
...
- Time stays dense inside the 30-day window (one slice per pentad); a log-spaced inner time ramp (1, 2, 3, 5, 8, 13 pentads at the same six slices) is an **ablation**, not the design — it would move the codec's window into stage 2's

*Notes.* Two choices are settled: the maps are at one degree with the coarse-plus-residual prior, and Argo keeps its nearest-profile tokens with no ellipse. Seven remain, each one line to change. The future cone goes to six pentads to mirror the past, where E-069 only asked for one and two. The task shares are a first guess. The strength of the prior on the fine map is one number, and the honest way to set it is by held-out loss on the training years rather than by hand — but the first arm may simply fix it. The v2 safety ring is kept at every lag, which is the safe and more expensive choice. The seasonal term on the drift waits for phase two. The harden interval and the geometry learning-rate multiplier are first guesses that every run records. And time stays dense inside the thirty-day window, one slice per pentad; a log-spaced time ramp that would stretch six slices over sixty-five days is listed as an ablation because it would move the codec's window into stage two's territory, which cone v2 deliberately separated in time. Say the word on any of these and the spec changes; none of them affects the build order.
...
## Slide 21 · The deck on one slide — with room for the result

**An hourglass that reads the past through a cone it shapes itself, tested three ways against its own fixed twin**

[FIGURE: three columns, each a picture over its text, because this slide also travels alone and its reader has not seen the deck. Column 1, "the hourglass": the space-time picture of slide 3 (hourglass_spacetime — past cone, waist, mirrored future cone) over the design card. Column 2, "the cone the model shapes": a compact re-arrangement of slide 8's warped sunflower (summary_cone — top row: (a) the canonical 24-point sunflower on the unit disc and (b) the same rotated and stretched to an ellipse with "S = log Σ = m·I + k·[cos 2θ, sin 2θ; sin 2θ, −cos 2θ]" beneath; bottom row: (c) the footprint at lags 1, 3, 6, shifted by ℓ·d and widened with ℓ, anchor at the origin, arrow "d — toward the source") over the cone card. Column 3, wider: an empty, dashed-outline panel labelled "RESULTS — figure to be inserted" with a one-line caption naming what belongs there ("R1 · future-cone loss per lead, A0 fixed / A1 learned / A2 primed, 3 seeds; R3 · learned drift vs upstream; R4 · the western-boundary depth check"), nothing drawn inside it, over the guards card and the experiment-and-verdict card. The results panel is smaller than in the first version of this slide to make room for the two pictures.]

- **The design** (slides 3–7). The present is a **waist** of ~13 cells, not a single tip; the past cone reads the last 30 days at every pentad; a **prediction cone** — the past cone mirrored through the anchor, +1…+6 pentads — is sampled the same way and used only as targets. Each example draws one of **four tasks**: forecast from waist + past (0.35), forecast from the waist alone (0.15), nowcast the hidden present from the past under five dropout patterns (0.25), fill-in (0.25) — under E-069b's rules: knowable targets only, no copy targets, every loss family scored against its own predict-the-mean bar
- **The cone the model shapes** (slides 8–12). A fixed 24-point sunflower, phase-rotated by the golden angle per lag, warped by **eight dimensionless numbers per 1° cell and channel group**: a drift **d** toward the source (capped at cone v2's design speed) and two ellipses Σ₀, Σ_v stored as **matrix logarithms** (no kilometres, no angle to wrap), the cone at lag ℓ being Σ₀ + ℓ²Σ_v. **One direction per flow, one scale and one memory per channel**: s_c and τ_c inflate or shrink the shared ellipse and set how many lags the channel reads, applied as a Gaussian **aperture** weight over the group's shared per-location tokens — zero extra tokens. Learned **gate-first** (soft aperture over fixed candidate dots, so the loader's reads never depend on live weights), then **hardened** (dot tables re-baked at epoch boundaries)
- **The guards** (slides 13, 15). Coarse-10°-plus-1°-residual prior; log-space soft floors; mirror tie; stop-gradient on target positions; evaluation geometry frozen for every arm; the far ring kept and never gated; the aperture never touches the loss weights — so the cone cannot learn to ask easy questions
- **The experiment** (slide 17), pre-registered: **A0** fixed hourglass (control and eval geometry) · **A1** learned from A0's init · **A2** learned, surface drift primed at −u_clim·Δt · same tensor (family 7.2), same 7 M codec, 20 k steps, frozen protocol, **3 seeds each**. Read-outs R1–R5; expected at the RAPID line: surface drift south-west, attention on deep profiles north. **Verdict**: A1 adopted if it beats A0 on R1 at every lead ≤ 3, paired at all seeds, R5 clean; A2 > A1 ≈ A0 → geometry set from climatology; neither → keep A0, next design is a per-example cone

*Notes.* This is the whole deck on one slide, for anyone who reads nothing else, and it is where the result goes when there is one. The left half is the argument in four steps. First, the design of the input and the targets: the present is a waist of about thirteen cells rather than a single point, because gradients need neighbours; the past is read at every pentad of the last thirty days; the future is a mirror image of the past cone, sampled identically and used only as targets; and each training example draws one of four tasks — forecast from everything, forecast from the present alone, nowcast the hidden present from the past, or fill in — under the three rules the E-069 experiment taught us (E-069 is the cone codec as built and measured at five seeds; the rules are that a target must be knowable from the input, must not be a copy of an input, and every loss family is scored against a predict-the-mean bar of its own). Second, the cone the model shapes: a fixed sunflower pattern, pushed through eight learned numbers per one-degree cell and channel group — a drift toward the source and two ellipses, stored as matrix logarithms so nothing is in kilometres and no angle wraps; the direction is shared by everything the water carries, while each channel has its own scale and its own memory, applied as a weight over shared tokens so it costs nothing; and the geometry is learned first through a soft weighting over fixed dots and only then baked into the dot table, so the data loader never has to know where the geometry currently is. Third, the guards, which exist because a learnable cone can learn to ask easy questions: the prior, the floors, the mirror tie, the stop-gradient on target positions, the frozen evaluation geometry, the ungated safety ring, and the rule that the aperture never reaches the loss weights. Fourth, the experiment as pre-registered: three arms — fixed, learned, and learned but primed with the climatological current reversed — on the global family 7.2 tensor (the first tensor covering the whole globe at quarter-degree resolution and five-day cadence, in its 7.2 build), the seven-million-parameter codec, twenty thousand steps, the frozen protocol with its held-out years, three seeds each; five read-outs; and the verdict rule written before any run. Two pictures carry the argument for a reader who has not seen the deck: the hourglass in space-time — the past cone narrowing to the waist and opening again into the mirrored future cone — and the warped sunflower, the fixed dot pattern first stretched into an ellipse and then drifting lag by lag toward the source. The third panel is deliberately empty. It is the space for the actual result — the forecast loss per lead for the three arms, the learned drift against the upstream direction, and the western-boundary depth check — and it stays empty until a run has produced those numbers, so that this slide can never show a picture of what we expected in place of what we measured. When the figure lands, the caption under the panel says which read-outs it shows, and the verdict line on the left is read against it.

---

## Slide 22 · The same hourglass at two scales
...
[FIGURE: two_scales_3d — two three-dimensional space-time drawings side by side, in the style of the warped-sunflower picture of slides 8 and 21: time runs upward, the two horizontal axes are kilometres, and every lag is a sunflower footprint — the 24-dot pattern inside its ellipse — stacked into a mirrored double cone. Past footprints below the present in blue ("input"), the waist at lag 0 in gold ("present"), future footprints above in orange ("targets"); the past footprints drift toward the source (the arrow d), the future ones are the point mirror. Left, "stage 1 — the codec, raw values": lags −6…+6 pentads (30 days each way), reach ~900 km at lag 6 (family B, 129.6 km·(1+ℓ)), the 13-cell waist; a few past dots hollow with a small "held out" tag. Right, "stage 2 — the forecaster, embeddings": the same double cone at stage 2's scale — footprints every dozen lags from −143 to +143 pentads (two years each way), opening to the 4,444 km cap, each dot an embedding rather than a raw value; the stage-1 double cone drawn to scale at the centre as a small dark shape, the near field the codec already read. Axis ticks: pentads on the vertical (±6 / ±143), kilometres on the horizontal (±1,000 / ±4,444), so the reader sees the factor. Beneath both a legend strip with three chips.]

- **Past cone** (blue): input. Never a forecast target. Sometimes **held out** — a dropout pattern hides a lag band, a bearing wedge, random dots or a channel (slide 6); the only thing ever asked of a hidden past dot is to be **filled back in** (T4)
- **Waist** (gold): the present, several cells. **Passed as input** in T1, T2 and T4; **hidden and predicted** in T3, the nowcast from history
- **Future cone** (orange): **targets only**, never input — the past cone point-mirrored through the anchor (slide 5)
- **Stage 1** (left): the codec, over raw values — lags 1…6 pentads, ~900 km at lag 6 for the ocean surface, the 13-cell waist; it writes one embedding per place and pentad
- **Stage 2** (right): the forecaster, over those embeddings — the same shape with the codec's near field cut out: lags 7…143 pentads (two years) to the 4,444 km cap of [`ml/cone.py::outer_spiral`](https://github.com/blauewelt/earth/blob/main/ml/cone.py), the present embedding as the waist, and a mirrored future cone of embeddings as targets. **The mirrored future side is the E-080 shape carried to stage 2 — a design intent; the stage-2 heads scored so far predict the next embedding and roll it forward** ([E-071 §4.5](https://blauewelt.github.io/earth/docs.html?f=ml/plans/E071_cone_v2.md))

*Notes.* One shape, two scales. On the left, stage one: the codec, the encoder that turns one place at one time into an embedding. It reads raw values through the small hourglass — the past cone of the last thirty days, six pentads deep and about nine hundred kilometres wide at its mouth for the ocean-surface channels; the waist of about thirteen cells at the present; and the mirrored future cone, six pentads ahead, which is only ever a target. Three rules, colour-coded and true at both scales. The past cone is input and never a forecast target; it is sometimes held out — a dropout pattern hides the most recent lags, a wedge of bearings, random dots, or one channel — and the one thing the model is ever asked to do with a hidden past dot is to fill it back in, which is task four, the reconstruction family kept from E-069b. The waist is the present: it is passed as input in the forecast tasks and in fill-in, and hidden and predicted in task three, the nowcast from history. The future cone is targets only and never input. On the right, stage two: the forecaster, which reads what stage one wrote. Each dot in its wide cone is an embedding, not a raw value, and the hourglass is the same shape drawn about twenty times longer and five times wider — lags seven to a hundred and forty-three pentads, two years, opening to the four-thousand-four-hundred-kilometre cap of the outer spiral, with the near field the codec already read cut out of the centre; that cut-out, drawn to scale, is the whole of stage one's hourglass. The present embedding is the waist. The future side, mirrored like stage one's, is the E-080 idea carried up a level: predict a cone of future embeddings rather than a single next step. That is stated here as the design intent; the stage-two heads the programme has scored so far predict the next embedding and roll it forward, one step at a time, over the long window that cone v2 assigns to stage two. Making stage two's targets a mirrored cone is a change to the forecaster's training, not to the codec, and it is not part of the E-080 experiment as pre-registered.
