...
[FIGURE: two_scales — two space-time panels side by side, same drawing conventions as slide 3, both with the three regions coloured the same way: past cone blue ("input"), waist gold ("present"), future cone orange ("targets"). Left panel, "stage 1 — the codec, raw values": the hourglass of slide 3 — past cone lags 1…6 pentads (30 days) opening to ~900 km at lag 6 (family B, 129.6 km·(1+ℓ)), the waist of ~13 cells at lag 0, the mirrored future cone +1…+6. In the past cone a few dots are drawn hollow with a small "held out" tag — the dropout patterns. Right panel, "stage 2 — the forecaster, embeddings": the same hourglass at stage 2's scale — past cone lags 7…143 pentads (two years) opening to the 4,444 km cap, the waist at lag 0, the future cone mirrored; drawn in the centre, to scale, the stage-1 hourglass as a small dark cut-out labelled "stage 1, to scale" — the near field the codec already read, which stage 2's spiral excludes. The vertical axes carry pentad ticks (±6 on the left, ±143 on the right) and the horizontal axes kilometres (±1,000 on the left, ±5,000 on the right), so the reader sees the factor. Beneath both panels a legend strip with three chips and one-line meanings.]
