# E-073 · Parallel import of the training data — the design (Apache Beam, polite by construction)
...
Companion files, all under `ml/import_beam/` in this repo: [the howto for the operating agent](https://github.com/blauewelt/earth/blob/main/ml/import_beam/README_FOR_GEMINI.md) (`README_FOR_GEMINI.md`), [the registry](https://github.com/blauewelt/earth/blob/main/ml/import_beam/sources.yaml) (`sources.yaml` — every source, every host budget), the code under `beam_import/`, the tests, and [the one-slide design](https://github.com/blauewelt/earth/blob/main/ml/import_beam/slide/E073_parallel_import_design.png). The credentials file travels only in the hand-over zip, never in the repo.
