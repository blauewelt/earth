...
// 12. Takeaways for Earth 2
{
  ...
  const cards = [
    ["Axis 1 — pixel vs. patch", "TESSERA proves a per-pixel encoder with zero spatial context is competitive on land; every other model buys context with a ViT patch grid. Our attribution matrix: single-pixel raw 0.613 vs codec 0.617; 3×3 raw 0.659 vs codec 0.672 (two seeds). Probe numbers move by 0.04–0.25 between seeds, so the neighbourhood's ≈ +0.05 is at the edge of the noise band and pretraining's ≈ +0.01 inside it — the former probably real, the latter unresolved.", SPACE],
...
// 12c–12d. Velocity-specific dependency cones (proposal from Chris, 31 Aug)
{
  const s = pres.addSlide(); s.background = { color: WHITE };
...

{
  ...
  const rows = [
    ...
    [nm("Transport target (RAPID 26°N)"), c("thermal wind = east–west density difference across the basin; set by interior Rossby adjustment (yrs) and by fast boundary waves (weeks)"), c("the whole section, both boundaries"), c("years for the interior arm; the current step for the boundary arm"), c("the read-out must see the whole section, not its spatial mean, which averages away the east–west contrast: the target's cone is the section (mechanism; the measured pooled-vs-unpooled gap is not quotable as evidence)")],
  ];
  ...
  s.addText([
    ...
    { text: "4  A pentad stage-2 will show one-step wind skill the monthly one cannot, because Δt ≈ τ_atm — untested: the earlier pentad rolls were withdrawn with the paper reset.", options: {} },
...
// 12e–12f. Related work for the cone approach and the multi-sphere approach
{
  ...
  const rows = [
    ...
    [nm("Earth 2 dependency-cone note (16 Aug) + this deck"), c("Cone recursion shown bit-equal to a full roll with NaN poisoning (a property of the roll code, not a skill number); reach 222 vs 4444 km per month covers 29.5 % vs 100 % of the world ocean after 12 months (geometry); now: per-process (v, τ) cones, union rule, channel-group stencils."), yes("—"), c("Open: the τ-truncation + union → per-channel stencil principle was not found stated anywhere in our search; it is a hypothesis to test (four ablations), not a result")],
  ];
