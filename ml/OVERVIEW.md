...
| **#527** E-066 · the LIM baseline — K 50/100/200 linear inverse model on the anomaly field, rolled from #516's starts and scored by the battery's own functions | how much of the head's rolled skill does a single linear operator already have — is the LIM the programme's reference model? | per lead / scope / channel against #516 (and #520/#523); head above the LIM at leads ≥ 2 by more than its tier spread ⇒ proceed; LIM at or above the head ⇒ LIM is the reference (REBOOT_PLAN step 6) | gpu-box-48647862 (Vast 49632487, Germany), dispatched 11:0xZ 09-02, CPU-only, lands in ~2 h; code `ml/lim_baseline.py` |
