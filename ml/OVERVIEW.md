...
| **#525** E-065a · continuous d_z-8 pentad codec, 200k×512 — the 24-bit lattice's warm-start parent | can a wider-bottleneck continuous codec be built so a [8]×8 lattice can be warm-started on it the E-050 way? | `loss_rec` / probe_head against run-480 (d_z 6: 0.229 / 0.579) and run-415 (d_z 32) — a parent worse than run-480 has a training fault and its lattice is not built | gpu-box-48397639 (Vast 49633425, Poland), dispatched 10:09Z 09-02 — **SLOW BOX: 38 min install, 1 h 52 m data-cache pull, in `Build dataset` since 12:40Z with no live branch at 14:00Z**; written deadline: no `ml-live-525` record by 15:15Z ⇒ cancel and re-dispatch on a fresh box (~$1 lost); ~14 h + probes once training; [plan](https://blauewelt.github.io/earth/docs.html?f=ml/plans/E065_fsq_ladder.md) |
