...
0a. **E-070 · the global tensor (family 7) — PLANNED 2026-09-03, Phase A (the four-lane GLORYS12 pull) RUNNING** — [plan](https://blauewelt.github.io/earth/docs.html?f=ml/plans/E070_global_tensor.md); Phases B–F wait on the pull and on E-069's NA r3 seeds
0. **E-069 · two stencils, one cone — BUILT 2026-09-02, NOT DISPATCHED** — [handover](https://blauewelt.github.io/earth/docs.html?f=ml/plans/E069_HANDOVER.md) written; JAX port in progress
   (Chris's ask the same day). TL;DR: does putting the inner dependency cone
   ...
   (`family4_na025_pentad_r3` = r2 + `cur_u`, `cur_v`), cone geometry,
   sampler, `ConeMAE` codec, trainer and 42 tests exist; the CPU smoke's
   velocity probe reads +0.073 (cone) vs −0.015 (snapshot) on a planted flow.
   Waits on: workflow wiring for r3 and the cone flags, then the codec
   dispatch (H1 first — cheap — before the five-seed stage-2 arms for H2/H3).
