...
\large a dataset design note --- Google's AlphaEarth annual 10\,m embeddings in place of the raw imagery}
...
\begin{abstract}
Family 0.9.tf is the alternative to family 1.0.tf's image catalogue: instead
of learning a local codec over petabytes of raw optical, radar and LiDAR
scenes, it ingests the embedding Google already computed from the same optical and
radar imagery, trained against the same LiDAR, L-band radar and climate
sources the catalogue lists --- the AlphaEarth Foundations Satellite
Embedding, version 1:
one 64-dimensional unit-length vector per 10\,m pixel per year, 2017--2025,
for every land surface and shallow water, published under CC BY 4.0, 64\,TB
...
says what the embedding is and what it was trained on, how it enters family
10's token without leaking a year's data into its own anchors, three stores
...
(E-078c). Google's AlphaEarth Foundations model has already done that job
at planetary scale: it reads the same optical and C-band radar imagery the
catalogue lists (Sentinel-2, Sentinel-1, Landsat 8 and 9) and was trained
...
a token source, so that the land side of ``predict everything'' can be
...
merged into it. If AlphaEarth tokens predict the land targets as well as the
...
year, or because the closed model cannot see NISAR, or because 2017 is too
late --- the comparison says exactly where 1.0.tf's raw record earns its
cost.

\section{What the embedding is}

Checked against the paper (Brown et al.\ 2025, arXiv 2507.22291, v2
2025-09-08), the Earth Engine catalogue entry, Google's storage read-me and
...
them at run time, are ALOS PALSAR-2 L-band radar, GEDI LiDAR, ERA5-Land
climate, GRACE gravity, the Copernicus GLO-30 elevation model, NLCD
land-cover labels, and Wikipedia articles linked to GBIF species
occurrences --- more than three billion observations in all, over about
...
\paragraph{The files.} Cloud-optimised GeoTIFFs of $8192 \times 8192$ pixels
with 64 bands, in local UTM projections, 120 zone directories per year,
about 34,000 tiles a year, 1.9\,GB on average. Values are \textbf{int8}
with nodata $-128$ and the documented de-quantisation
\begin{equation}
...
\paragraph{Where it lives, measured.} \code{gs://alphaearth\_foundations/}\allowbreak
\code{satellite\_embedding/v1/annual/<year>/<zone>/}, with index files
(\code{aef\_index.parquet}, 70\,MB, updated 2026-02-27); the bucket has been
provider-pays since July 2026, so a copy costs the reader nothing. A
complete mirror on Source Cooperative (\code{tge-labs/aef}, AWS Open Data,
\code{s3://us-west-2.opendata.source.coop/tge-labs/aef/v1/annual/}, anonymous
S3 and HTTPS) has the same tile counts and byte totals as the origin for
every year, plus one VRT per tile; its read-me warns that the COGs are
stored bottom-up, so they are read through the VRTs. Earth Engine serves the
same layers as floats in $[-1, 1]$ (bands \code{A00}--\code{A63}).
...
Every store below carries it; \code{licence.redistribution} is
\code{attribution} and \code{distribution} is \code{public}. The two-track
rule of family 1.gf \S4 applies, and nothing in this family needs the
private track today.
...
private-track store, \code{aef\_custom} (family 1.gf \S4), never
republished --- the one place this family would use that track. The model and training code are
...
\section{Grid, time axis and token}
...
copied (\S4.4), so nothing is resampled at storage time except what is
declared as a pooled field with its own footprint.

\paragraph{Time.} A year-$Y$ embedding summarises 1 January to 31 December
of $Y$: $\log_2 f_t = \log_2(365/5) = +6.2$. It is filed under the
\textbf{first family-10 bin that begins after 31 December $Y$} --- not the
...
\paragraph{Footprint.} A raw 10\,m vector carries $\log_2 f_p =
...
$C = 64$ (plus one coherence channel for pooled stores, \S4.1), raw units
...
everywhere else. Nothing is z-scored: the components are already on a
...
\subsection{\code{aef1k} --- the pooled 1\,km field with coherence, tier G sharded}
...
\subsection{\code{aef\_dots} --- raw 10\,m vectors at fixed sub-cell positions, tier P}
...
year at fixed sub-cell positions --- the cone's own sunflower offsets scaled
...
float16 ($C = 64$; int8 would need a schema change and is not worth it at
...
\subsection{\code{aef100r} --- 100\,m over regions of interest, tier G sharded}
...
\subsection{\code{aef10} --- the original, referenced by byte range}
...
bucket, the same operation the family-7 globe layer performs on the Hub,
...
\midrule
\code{aef1k} & G sharded & 64 pooled + 1 coherence, int8/uint8, $1/120^\circ$, one frame per year & $(-4.9, +6.2)$ & 270\,M land cells $\times$ 9 & $\approx 0.2$\,TB & A \\
\code{aef\_dots} & P & 16 raw 10\,m vectors per $0.25^\circ$ land cell per year, float16 & $(-11.4, +6.2)$ & 43\,M rows & 7\,GB & A \\
\code{aef100r} & G sharded & pooled 100\,m over regions of interest; the coastal band later & $(-8.2, +6.2)$ & $5 \times 10^8$ cells $\times$ 9 & $\approx 0.35$\,TB (regions); $\approx 5.7$\,TB (band) & B / C \\
\code{aef10} & external & the mirror's COGs by byte range; index and formula in the registry & $(-11.4, +6.2)$ & $1.4 \times 10^{12}$ px $\times$ 9 & 0 (577\,TB referenced) & A \\
\bottomrule
\end{tabular}
\caption{The four groups of family 0.9.tf.}
\end{table}

\section{Missing values}

Nodata ($-128$ in the source) is NaN in every store and never a zero
...
absent is what the token says ($n_R = 0$).
...
\code{slatrack} night; on a spot instance in the mirror's own region
(us-west-2, in-region S3 reads free, anonymous) it is about five days at
10\,Gbps and \$50--150 of compute, writing year-parts to the Hub as it goes
with \code{done.json} last, the family-10 lane discipline. Pick: in-region,
...
CSR index) and family 1.gf (shards decompressing to declared shapes); plus
...
the table above, so a truncated year is refused by name.
...
Nothing is dispatched on an unmeasured size: the first year's build
measures the pooling throughput, the coherence distribution and the dots'
bytes before the rest.

\section{Next steps: the ladder, the probe and the falsifiers}

\paragraph{Phase A.} \code{aef1k}, \code{aef\_dots}, the \code{aef10}
reference group and the registry; $\approx 0.2$\,TB; one in-region lane per
...
\paragraph{The probe.} One UTM zone of one year (about 280 tiles, 0.5\,TB)
...
The Hugging Face sample (\code{Major-TOM/Core-AlphaEarth-Embeddings}, 62.5\,k
grid cells, 2017--2023) is the unit-test fixture.

\paragraph{Three falsifiers, three seeds each.}
...
target stores) predicted from family 7.2 alone versus family 7.2 plus this
family's tokens, at the targets' own footprints. Parity means 10\,m
information does not reach those targets through an annual summary.
\item \textbf{What the annual cadence costs.} The same targets from this
family plus family 1.0.tf's daily fields (temperature, reflectance, snow,
fire) versus this family alone. The gap is the value of the five-day rule on
