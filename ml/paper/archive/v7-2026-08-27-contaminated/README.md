...
that established this are written up as §14 of this archived report
("Addendum: the training pool leaked the held-out years", added
2026-09-02; figures from `make_figs_addendum.py`, data from
`../../data/report_data.json`) and in `ml/plans/PROTOCOL_RESET.md` §2. The
current report at `ml/paper/paper.tex` carries only results measured under
the corrected protocol and, by standing rule (root `CLAUDE.md` §6b), none of
this history.
...
(dark, generated by `build.sh`), `make_figs.py` and the JSON inputs it read,
`make_figs_addendum.py`, `figs/` and `figs_dark/`. Rebuild with
