...
throughout. A 7.6M head rolled through the same battery has the same mean
correlation as the 206.7M head ($0.103$ against $0.105$) and does not decay
through zero, holding $\approx 0.11$ from two months to a year; its
squared-error score is less negative ($-0.18$) only because it emits smaller
anomalies. Transport at $26.5^\circ$N shows no measurable skill at roughly
...
\caption{Left: corridor anomaly correlation per lead for the 206.659M and
7.598M heads and linear inverse models with 50, 100 and 200 modes, from the
same starts.
...
\paragraph{Width on the rolled field.} The 7.598M head ($256\times8$,
K$=144$, its 20{,}000-step end state) rolled through the identical battery
scores the same field as the 206.659M head (Table~\ref{tab:width},
Figure~\ref{fig:lim}): mean corridor correlation $0.103$ against $0.105$,
and the same amplitude-calibrated ceiling (mean $\mathrm{ACC}^2$ $0.0185$
against $0.0188$). The two differ at the ends of the lead axis. The larger
head is more correlated over the first ten days ($0.606$ and $0.412$ against
$0.569$ and $0.403$); the smaller one does not decay through zero, holding
$0.10$--$0.14$ from 60 days to a year where the larger falls from $0.13$ to
$-0.03$. Its $\mathrm{MSSS}_{\text{clim}}$ is less negative at every lead
past ten days (mean $-0.179$ against $-0.439$) because it emits anomalies at
$a\approx0.54$ rather than $0.78$, and $a$ does not grow with lead; by the
identity above, at $\mathrm{ACC}\approx0.1$ the smaller amplitude is the
cheaper error. Sea-surface temperature is again the channel that carries the
skill (mean $\mathrm{MSSS}_{\text{clim}}$ $-0.006$, positive at 33 of 73
leads, against $-0.165$ and 11 of 73 for the large head); sea-surface height
is the sharpest one-step channel for both ($0.87$ and $0.84$). Both heads
beat raw persistence at nearly every lead and damped persistence at none,
and the linear inverse model is more correlated than either from fifteen
days on. The transport bands of the two heads have opposite signs
($-0.025$, $+0.103$, $-0.244$ against $+0.107$, $-0.242$, $+0.163$). The
roll took 7{,}660\,s against 77{,}147\,s. One seed each; the 40.388M rung
is not yet rolled.
...
\toprule
 & \multicolumn{2}{c}{ACC} & \multicolumn{2}{c}{$\mathrm{MSSS}_{\text{clim}}$} & \multicolumn{2}{c}{$a$} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}\cmidrule(lr){6-7}
lead (days) & 7.598M & 206.659M & 7.598M & 206.659M & 7.598M & 206.659M \\
\midrule
...
\bottomrule
\end{tabular}
\caption{The 7.598M and 206.659M heads through the same battery, corridor
scope. Mean $\mathrm{MSSS}_{\text{damped}}$ $-0.200$ and $-0.461$; gate and
window mean $\mathrm{MSSS}_{\text{clim}}$ $-0.201$/$-0.213$ and
$-0.395$/$-0.401$. Single seed each.\label{tab:width}}
\end{table}
...
the same field at 7.6M parameters as at 206.7M, forecasts sea-surface
...
with block-bootstrap intervals; the early-stopped checkpoint and the 40.4M
head rolled against the end states shown here; amplitude
...
(\path{probes-<n>.json}; the rolls here are \path{probes-516.json} and \path{probes-520.json}, the linear inverse model \path{probes-527.json} (\path{ml/lim_baseline.py}), the
