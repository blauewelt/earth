...
throughout. Transport at $26.5^\circ$N shows no measurable skill at roughly
...
\caption{Left: corridor anomaly correlation per lead for the 206.659M head
and linear inverse models with 50, 100 and 200 modes, from the same starts.
...
sea-surface temperature and height for days to weeks, does not beat a
damped-persistence null at any lead, and is out-forecast beyond ten days by
a linear inverse model with 200 modes fitted to the same field. The linear
...
with block-bootstrap intervals; the early-stopped checkpoint and the 7.6M
and 40.4M heads rolled against the end state shown here; amplitude
...
(\path{probes-<n>.json}; the roll here is \path{probes-516.json}, the linear inverse model \path{probes-527.json} (\path{ml/lim_baseline.py}), the
