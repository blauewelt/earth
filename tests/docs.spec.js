...
test.describe("docs.html · mobile markdown reader", () => {
  ...
  /* The run index is generated by scripts/run_index.mjs and is the ONLY
   * durable answer to "what was #386?" once that run falls out of the status
   * page's window. Registering it is part of writing it (SS0b). */
  test("the generated run index is registered and reachable", async ({ page }) => {
    await page.goto("/docs.html");
    const hrefs = await page.locator(".idx a").evaluateAll((as) =>
      as.map((a) => decodeURIComponent(a.getAttribute("href") || "")));
    expect(hrefs).toContain("?f=ml/RUNS.md");
  });

  /* MEASURED, not assumed: status.html's experiment badge and the run index's
   * own permalinks both hang on docs.html resolving a #fragment against the
   * RENDERED markdown, which happens after an async fetch. If that ever stops
   * working, every deep link in the project silently degrades to "top of a
   * 2,400-line file" — which is exactly the phone experience SS0b exists to
   * prevent, and it would look like nothing at all had gone wrong. */
  test("a #fragment resolves against the rendered markdown", async ({ page }) => {
    // an explicit <a id="…"> anchor, the kind EXPERIMENTS.md carries
    await page.goto("/docs.html?f=ml/EXPERIMENTS.md#e-022");
    await expect(page.locator("main h1, main h2").first()).toBeVisible();
    const exp = await page.evaluate(() => {
      const el = document.getElementById("e-022");
      return el ? { found: true, top: el.getBoundingClientRect().top } : { found: false };
    });
    expect(exp.found, "EXPERIMENTS.md#e-022 did not resolve").toBe(true);
    expect(Math.abs(exp.top), "the page did not scroll to it").toBeLessThan(200);

    // and the run index's per-row anchor, which is what makes
    // docs.html?f=ml/RUNS.md#run-413 a usable link from a session report
    await page.goto("/docs.html?f=ml/RUNS.md#run-413");
    await expect(page.locator("main h1").first()).toBeVisible();
    const run = await page.evaluate(() => {
      const el = document.getElementById("run-413");
      return el ? { found: true, text: el.textContent.trim(),
                    top: el.getBoundingClientRect().top } : { found: false };
    });
    expect(run.found, "RUNS.md#run-413 did not resolve").toBe(true);
    expect(run.text).toBe("#413");
    expect(Math.abs(run.top), "the page did not scroll to run #413").toBeLessThan(200);
  });

  /* MEASURED 2026-08-19: status.html's experiment badge links 54 distinct
   * experiment IDs, and only 19 of them have an explicit <a id> in
   * EXPERIMENTS.md. A bare getElementById therefore dropped two thirds of
   * those links at the top of a 5,000-line document — which looks to the
   * reader exactly like a broken link and to the author exactly like a
   * working one. So the lookup falls back to the heading slug (headings
   * open with the ID), then to the parent experiment for an arm, and
   * finally admits it found nothing. */
  test("an experiment ID with no explicit anchor still lands on its section",
    async ({ page }) => {
      const fs = require("fs"), path = require("path");
      const md = fs.readFileSync(
        path.join(__dirname, "..", "ml", "EXPERIMENTS.md"), "utf8");
      // pick IDs the document does NOT anchor explicitly, so the test is
      // about the fallback and not about an anchor that happens to exist
      const explicit = new Set(
        [...md.matchAll(/<a id="(e-[\w-]+)"/g)].map((m) => m[1]));
      const heading = [...md.matchAll(/^#{2,3} +(E-\d{3}[a-z]?)\b/gm)]
        .map((m) => m[1].toLowerCase());
      const fallbacks = [...new Set(heading)].filter((id) => !explicit.has(id));
      expect(fallbacks.length,
        "no un-anchored experiment heading left to test the fallback on")
        .toBeGreaterThan(0);

      for (const id of fallbacks.slice(0, 4)) {
        await page.goto("/docs.html?f=ml/EXPERIMENTS.md#" + id);
        await expect(page.locator("main h1, main h2").first()).toBeVisible();
        const r = await page.evaluate((wanted) => {
          const miss = document.getElementById("fragmiss");
          return { missed: !!miss, y: Math.round(window.scrollY), wanted };
        }, id);
        expect(r.missed, id + " fell through to the 'no such section' note")
          .toBe(false);
        expect(r.y, id + " did not scroll anywhere").toBeGreaterThan(0);
      }
    });

  test("a fragment the document really lacks says so instead of sitting at the top",
    async ({ page }) => {
      await page.goto("/docs.html?f=ml/EXPERIMENTS.md#e-999");
      const note = page.locator("#fragmiss");
      await expect(note).toBeVisible();
      await expect(note).toContainText("e-999");
      await expect(note).toContainText("no section");
    });
